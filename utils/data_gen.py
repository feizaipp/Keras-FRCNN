...
def format_img_size(width, height, img_min_size=600):
    if width <= height:
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_func):
        ...
        for anchor_ratio_idx in range(n_anchratios):
            ...
            for ix in range(output_width):
                ...
                for jy in range(output_height):
                    y1_anc = downscale * (jy + 0.5) - anchor_y
                    y2_anc = downscale * (jy + 0.5) + anchor_y
...
def get_anchor_gt(all_img_data, class_count, C, img_length_calc_func, backend, mode="train"):
    ...
    while True:
        ...
        for img_data in all_img_data:
            try:
                ...
                assert cols == width
                assert rows == height
                (resized_width, resized_height) = format_img_size(width, height, C.im_size)
                x_img = cv2.resize(x_img, (resized_width, resized_height), interpolation=cv2.INTER_CUBIC)
                
                try:
                    y_rpn_cls, y_rpn_regr = calc_rpn(C, img_data_aug, width, height, resized_width, resized_height, img_length_calc_func)
                except:
                    traceback.print_exc()
                    sys.exit(1)
                    #print(e)
